...
the code in catch-block is executed. */
    ...
    try {
        $sql = "DELETE FROM $dbname.CustomerVehicle WHERE LicensePlate = :lp";
        $stmnt = $conn->prepare($sql);     // read about prepared statements here: https://www.w3schools.com/php/php_mysql_prepared_statements.asp
        $stmnt->bindParam(':lp', $_POST['LicensePlate']);   // :stdId is the variable that we used in $sql, there must be a colon (:) in front of it.
